...
const TESTIMONIALS = [
  { name:'Chioma Okafor', role:'Medical Student, UNILAG', rating:5, text:'StudyPilot completely transformed how I prepare for exams. The AI timetable actually fits my real schedule and the readiness score keeps me accountable.', initials:'CO', color:'#7c3aed' },
  { name:'Emeka Nwachukwu', role:'Engineering Student, OAU', rating:5, text:'I went from failing two courses to making Dean\'s List in one semester. The AI knows exactly how much time I need and adjusts when I fall behind.', initials:'EN', color:'#db2777' },
  { name:'Fatima Al-Hassan', role:'Law Student, ABU', rating:5, text:'The analytics are incredible. I can see exactly where I spend study time and the AI gives weekly insights. Worth every single naira.', initials:'FA', color:'#6d28d9' },
]
              ...
              <motion.div variants={fadeUp} className="flex items-center gap-3">
                <div className="flex -space-x-2">
                  {COLORS.slice(0,4).map((c,i) => (
                    <div key={i} style={{ background:c }} className="w-9 h-9 rounded-full border-2 border-white flex items-center justify-center text-white text-xs font-bold shadow-sm">
                      {['A','B','C','D'][i]}
                    </div>
                  ))}
                </div>
            ...
            {[
              ...
              <div key={field.key}>
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  {field.key === 'password' && <button type="button" className="text-xs text-purple-600 font-semibold hover:text-purple-700">Forgot?</button>}
                </div>
                <input type={field.type} placeholder={field.ph} value={(f as any)[field.key]}
                  onChange={e => setF({ ...f, [field.key]: e.target.value })}
                  className={`w-full px-4 py-3 rounded-xl border text-sm outline-none transition-all placeholder:text-gray-300 ${
                    (err as any)[field.key] ? 'border-red-300 bg-red-50/50' : 'border-gray-200 bg-gray-50 focus:border-purple-400 focus:bg-white focus:ring-2 focus:ring-purple-100'
                  }`} />
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div>
              ...
              <input type="password" placeholder="Create a strong password" value={f.password}
                ...
                className={`w-full px-4 py-3 rounded-xl border text-sm outline-none transition-all placeholder:text-gray-300 ${
                  err.password ? 'border-red-300 bg-red-50/50' : 'border-gray-200 bg-gray-50 focus:border-purple-400 focus:bg-white focus:ring-2 focus:ring-purple-100'
                }`} />
              {f.password && (
            ...
            <div>
              ...
              <input type="password" placeholder="Repeat your password" value={f.confirm}
                ...
                className={`w-full px-4 py-3 rounded-xl border text-sm outline-none transition-all placeholder:text-gray-300 ${
                  err.confirm ? 'border-red-300 bg-red-50/50' : 'border-gray-200 bg-gray-50 focus:border-purple-400 focus:bg-white focus:ring-2 focus:ring-purple-100'
                }`} />
    ...
    <div className="flex flex-col h-full bg-white border-r border-gray-100 w-60">
        ...
        {nav.map(item => (
          <button key={item.id} onClick={() => { setTab(item.id as any); setMobSidebar(false) }}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all ${
              tab === item.id ? 'bg-purple-50 text-purple-700 border border-purple-100' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'
            }`}>
            <span>{item.icon}</span>
            {item.label}
      ...
      <div className="p-4 border-t border-gray-50">
        <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 cursor-pointer transition-colors">
          ...
          <div><p className="text-sm font-bold text-gray-900">Amaka Johnson</p><p className="text-xs text-purple-500 font-medium">✨ Pro Plan</p></div>
        </div>
      </div>
    </div>
  )
              ...
              {tasks.map((task: Task, i: number) => (
                <motion.div key={task.id} layout
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-semibold flex-shrink-0 ${
                    task.priority==='high' ? 'bg-red-50 text-red-500' : task.priority==='medium' ? 'bg-amber-50 text-amber-500' : 'bg-green-50 text-green-600'
                  }`}>{task.priority}</span>
                </motion.div>
              ))}
            </div>
  ...
  async function generate(e: React.FormEvent) {
    ...
    setPlan(days.map(day => ({
      ...
      sessions: valid.slice(0,2).map((s,i) => ({
        subject:s, time:`${8+i*2}:00 — ${10+i*2}:00`, color:COLORS[i%COLORS.length]
      }))
    })))
    setLoading(false)
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div><h2 className="font-black text-gray-900 text-xl tracking-tight">Tasks</h2>
